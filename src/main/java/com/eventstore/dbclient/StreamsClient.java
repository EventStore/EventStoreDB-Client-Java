...
import com.eventstore.dbclient.result.SuccessResult;
import com.eventstore.dbclient.result.WrongExpectedVersionResult;
    ...
    private CompletableFuture<WriteResult> appendInternal(
        ...
        StreamObserver<StreamsOuterClass.AppendReq> requestStream = _stub.append(convertSingleResponse(result, resp -> {
            if (resp.hasSuccess()) {
                ...
                return new SuccessResult(nextExpectedRevision, logPosition);
            }
            if (resp.hasWrongExpectedVersion()) {
                ...
                return new WrongExpectedVersionResult(options.getStreamName(), expectedRevision, currentRevision);
            }
