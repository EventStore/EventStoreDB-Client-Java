...
import com.eventstore.dbclient.proto.users.UsersOuterClass.EnableReq.Options;
...
class EnableUser {
    ...
    public EnableUser(GrpcClient client, String loginName) {
        this.client = client;
    ...
    public CompletableFuture<Void> execute() {
        return this.client.run(channel -> {
            ...
            users.enable(request(), convertSingleResponse(result, resp -> null));
            return result;
        });
    }

    private UsersOuterClass.EnableReq request() {
        Options options = Options.newBuilder().setLoginName(loginName).build();
        return UsersOuterClass.EnableReq.newBuilder().setOptions(options).build();
    }
